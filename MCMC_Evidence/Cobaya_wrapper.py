...
def match_CosmoMC_chains(root, params, verbose=False):
    ...
    if os.path.isfile(root + ".1.txt"):
        ...
        chain_1[keep_params].to_csv(
            root + "_BE.1.txt", header=False, index=False, sep=" "
        )
        if verbose:
    ...
    if os.path.isfile(root + ".2.txt"):
        ...
        chain_2[keep_params].to_csv(
            root + "_BE.2.txt", header=False, index=False, sep=" "
        )
        if verbose:
    ...
    if os.path.isfile(root + ".3.txt"):
        ...
        chain_3[keep_params].to_csv(
            root + "_BE.3.txt", header=False, index=False, sep=" "
        )
        if verbose:
    ...
    if os.path.isfile(root + ".4.txt"):
        ...
        chain_4[keep_params].to_csv(
            root + "_BE.4.txt", header=False, index=False, sep=" "
        )
        if verbose:
    ...
    if os.path.isfile(root + ".5.txt"):
        ...
        chain_5[keep_params].to_csv(
            root + "_BE.5.txt", header=False, index=False, sep=" "
        )
        if verbose:
    ...
    if n_chains == 0:
        ...
        if verbose == True:
            print(
                "\n",
                n_chains,
                "chains produced to mach the CosmoMC output with the following columns:\n",
            )
            print(keep_params)
            print("\n")
...
def get_dot_ranges(root, params, verbose=False):
    ...
    # for delimiter ; read from the .updated.yaml file the Gaussian priors
    with open(root + ".updated.yaml", "r") as f:
        cobaya_info = yaml.safe_load(f)
    cobaya_dict = cobaya_info.get("params", {})
    ...
    for i in range(len(params)):
        if ":" in params[i]:
            ...
            # should write the confidence level beyond which we assume the prior is 0.
            CL = params[i].split(";", 1)[1]
            _params[i] = params[i].split(";")[0]
            prior_info = cobaya_dict[_params[i]]["prior"]
            mu = float(prior_info["loc"])
            sigma = float(prior_info["scale"])
            lower[i] = norm.ppf((1 - float(CL)) / 2, loc=mu, scale=sigma)
            upper[i] = norm.ppf((1 + float(CL)) / 2, loc=mu, scale=sigma)
            try:
                logvolume = logvolume * np.abs(float(upper[i]) - float(lower[i]))
            except:
                pass
            if verbose == True:
                print("reading input priors", end=" ")
                print(params[i].split(";")[0], end=" ")
                print("Confidence for unbounded prior: ", end=" ")
                print(CL, end=" ")
                print("Corresponding bounds: ", end=" ")
                print(lower[i], end=" ")
                print(upper[i])
    ...
    for i in range(len(params)):
        ...
        if _params[i] == "omch2" or _params[i] == "omega_cdm":
            _params[i] = "omegach2"

        if _params[i] == "theta_MC_100" or _params[i] == "theta_s_100":
            _params[i] = "theta"
...
def MCMC_Evidence(
    root, params, burnlen=0.3, verbose=True, get_results=False, labels=False
):
    print("===========================================================")
    ...
    lnB = BayesianEvidence(root + "_BE", burnlen=burnlen)[0]
